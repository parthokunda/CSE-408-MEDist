...
const DoctorAppointmentCard: FC<{ app: PendingAppointmentOverviewInfo }> = (
  props
) => {
  // convert Date to date and time
  const navigate = useNavigate();
  // console.log("🚀 ~ file: DoctorAppoinmentCard.tsx:20 ~ app:", props.app);
        ...
        <CardContent className="p-2 px-0">
          <div className="grid grid-cols-6 gap-4 align-middle">
            ...
            {(props.app.status === AppointmentStatus.PRESCRIBED ||
              props.app.status === AppointmentStatus.COMPLETED) && (
              <div className="col-span-2 flex justify-center">
                <Button
                  className="bg-c2 w-42 text-white rounded-lg hover:bg-c1 m-2"
                  onClick={() =>
                    navigate(`/doctor/prescription/${props.app.id}`)
                  }
                >
                  View Prescription
                </Button>
